                                          ...
                                          const jerry_length_t argsNum)
{
    ...
    if (component->fontValue_ == nullptr) {
        ...
        if (nameLength > 0 && (nameLength < maxNameLen) && (defaultFontSize < UINT8_MAX)) {
            std::stringstream ss;
            ss << defaultFontSize << "px " << defaultFontName;
            if (ss.str().length() > 0 && ss.str().length() < maxBufferLen) {
                CopyFontFamily(component->fontValue_, ss.str().c_str(), ss.str().length());
            }
        }
    }
...
std::string CanvasComponent::GetImageObjectParam(const jerry_value_t args[], int16_t &width, int16_t &height)
{
    jerry_value_t src = jerryx_get_property_str(args[ArgsIndex::IDX_0], ImageModule::attrSrc);
    char* srcVal = MallocStringOf(src);
    std::string strName = srcVal;
    ACE_FREE(srcVal);
