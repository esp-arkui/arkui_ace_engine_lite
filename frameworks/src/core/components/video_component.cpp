...
bool VideoComponent::CreateNativeViews()
{
    ...
    return PrepareProgressUpdatingThread();
}
...
void VideoComponent::ReleaseNativeViews()
{
    ...
    const uint16_t waitThreadDestroyTime = 20000;
    while (!isUpdatingHandlerQuited_) {
        usleep(waitThreadDestroyTime);
    }
...
bool VideoComponent::RegisterPrivateEventListener(uint16_t eventTypeId, jerry_value_t funcValue, bool isStopPropagation)
{
...
void VideoComponent::OnViewAttached()
{
    if (videoView_ == nullptr) {
        return;
    }
...
/* separated thread to trigger all update progress request */
