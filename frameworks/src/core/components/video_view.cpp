...
bool VideoView::CreateVideoView()
{
    ...
    if ((videoPlayer_ == nullptr) || (surfaceView_ == nullptr) ||
        (panelView_ == nullptr) || !(panelView_->InitView())) {
        return false;
    }
    Add(surfaceView_);
    Add(panelView_);
...
{
    VideoView *videoView = static_cast<VideoView *>(arg);
...
bool VideoView::DestroyPlayer()
{
    if (videoPlayer_ == nullptr) {
        return false;
    }
    if (videoPlayer_->IsPlaying() && videoPlayer_->Stop() != 0) {
        HILOG_ERROR(HILOG_MODULE_ACE, "DestroyPlayer fail to stop videoplayer");
    }
    if (videoPlayer_->Reset() != 0) {
        HILOG_ERROR(HILOG_MODULE_ACE, "DestroyPlayer fail to reset videoplayer");
    }
    if (videoPlayer_->Release() != 0) {
        HILOG_ERROR(HILOG_MODULE_ACE, "DestroyPlayer fail to release videoplayer");
    }
    delete videoPlayer_;
    videoPlayer_ = nullptr;
...
{
    // update current play position time
    UpdatePanelTimeText(true);
}

void VideoView::SetCurrentPlayTimeText(int32_t inSeconds)
{
...
void VideoView::CallJSFunctionWithOnePara(const jerry_value_t &callBackFunc, const int64_t inputValue)
{
    ...
    jerry_value_t result = jerryx_set_property_str(args[0], "currenttime", value);
