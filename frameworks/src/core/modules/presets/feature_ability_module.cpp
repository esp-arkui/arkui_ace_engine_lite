                                           ...
                                           const jerry_length_t length)
{
    ...
    char *bundleName = const_cast<char *>(JsAppContext::GetInstance()->GetCurrentBundleName());
    int32_t ret = AbilityKit::DetectPhoneApp(bundleName, dstAbilityName, detectSuccessCallback_, detectFailCallback_,
                                             detectCallbackContext_);
    if (ret != 0) {
                                                  ...
                                                  const jerry_length_t length)
{
    ...
    if ((dstBundleName == nullptr) || (nameLength == 0) || (message == nullptr) || (msgLength == 0) ||
        ...
        char *bundleName = const_cast<char *>(JsAppContext::GetInstance()->GetCurrentBundleName());
        int32_t ret = AbilityKit::SendMsgToPeerApp(
            (idLength == 0), bundleName, dstBundleName, (reinterpret_cast<const uint8_t *>(message)), strlen(message),
            sendMsgSuccessCallback_, sendMsgFailCallback_, sendMsgCallbackContext_);
...
int32_t FeatureAbilityModule::MessageSuccessCallback(const void *data)
{
    ...
    FeatureAbilityDataInfo *target = static_cast<FeatureAbilityDataInfo *>(ace_malloc(sizeof(FeatureAbilityDataInfo)));
    if (target == nullptr) {
