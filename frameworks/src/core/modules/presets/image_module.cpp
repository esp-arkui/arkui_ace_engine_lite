                                       ...
                                       const jerry_length_t argsNum)
{
    ...
    if (imageModel->src_.empty() || imageModel->src_ == "") {
        OnCallBack(context, imageModel, false, "src is null");
        return UNDEFINED;
    }
    jerry_release_value(imageModel->jerrySrc_);
    imageModel->jerrySrc_ = jerry_create_string(reinterpret_cast<const jerry_char_t *>(imageModel->src_.c_str()));
    Image image;
    bool isLoad = image.PreParse(imageModel->src_.c_str());
    if (!isLoad) {
