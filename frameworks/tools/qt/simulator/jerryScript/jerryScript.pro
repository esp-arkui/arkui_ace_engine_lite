...
DEFINES += JERRY_CPOINTER_32_BIT=1
DEFINES += JERRY_DEBUGGER=1
DEFINES += JERRY_ES2015=1
DEFINES += JERRY_EXTERNAL_CONTEXT=1
DEFINES += JERRY_FUNCTION_BACKTRACE
DEFINES += JERRY_FUNCTION_NAME
DEFINES += JERRY_GC_LIMIT=1
DEFINES += JERRY_LINE_INFO=1
DEFINES += JERRY_MEM_GC_BEFORE_EACH_ALLOC=1
DEFINES += JERRY_NDEBUG
DEFINES += JERRY_PARSER=1
DEFINES += JERRY_PARSER_DUMP_BYTE_CODE=1
DEFINES += JERRY_REF_TRACKER
DEFINES += JERRY_REGEXP_DUMP_BYTE_CODE=1
DEFINES += JERRY_REGEXP_STRICT_MODE=1
DEFINES += JERRY_STACK_LIMIT=0  ##开启后报错，PARSER_ERR_JERRY_STACK_LIMIT_REACHED
DEFINES += JERRY_SYSTEM_ALLOCATOR=0 ##开启后编译不过
DEFINES += JERRY_VALGRIND=0 ##开启编译不过 include "memcheck.h" 找不到
DEFINES += JERRY_VM_EXEC_STOP=1
